"""Idempotent FAQ seed. Safe to run on any environment, safe to include in
the Render build command — existing FAQ rows are preserved and won't be
clobbered by admin edits. Only rows that don't already exist (matched on
audience + question) get created.

Same content as apps/admin_panel/migrations/0008_seed_faqs.py — copied
verbatim so this stays the single source of truth once the migration has
already run on an environment (Django won't re-run 0008 after that; a DB
flush wipes the rows but leaves the migration marked applied, so migrate
alone doesn't restore them).
"""
...
EMPLOYER_FAQS = [
    ...
    (
        ...
        "you can message the candidate, send requirements, arrange interviews, and eventually use <strong>Send Job Offer</strong>.",
    ),
    (
...
class Command(BaseCommand):
    help = 'Idempotently seed FAQ rows. Existing rows are left alone; only missing questions are created.'
    ...
    def handle(self, *args, **options):
        ...
        for i, (q, a) in enumerate(JOBSEEKER_FAQS):
            _, was_created = FAQ.objects.get_or_create(
                audience=FAQ.AUDIENCE_JOBSEEKER, question=q,
                defaults={'answer': a, 'order': i, 'is_published': True},
            )
            created += 1 if was_created else 0
            skipped += 0 if was_created else 1
        for i, (q, a) in enumerate(EMPLOYER_FAQS):
            _, was_created = FAQ.objects.get_or_create(
                audience=FAQ.AUDIENCE_EMPLOYER, question=q,
                defaults={'answer': a, 'order': i, 'is_published': True},
            )
            created += 1 if was_created else 0
            skipped += 0 if was_created else 1
        self.stdout.write(self.style.SUCCESS(
            f'FAQs seeded — {created} created, {skipped} already present (kept intact).'
        ))
